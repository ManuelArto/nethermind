...
public sealed class ZkNewPayloadHandler : IAsyncHandler<ExecutionPayload, PayloadStatusV1>
{
    ...
    public ZkNewPayloadHandler(IBlockTree blockTree, IPoSSwitcher poSSwitcher, ILogManager logManager)
    {
    ...
    private async Task<(ValidationResult, string?)> ValidateAsync(Block block)
    {
        ...
        if (result is BlockValidator.ZkValidationResult.Invalid)
        {
        ...
        } else if (result is BlockValidator.ZkValidationResult.Unavailable)
        {
